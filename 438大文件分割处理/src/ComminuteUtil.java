...
public void fenGe(File commFile, File untieFile, int filesize) {
    ...
    long size = 1024 * 1024; // 用来指定分割文件要以MB为单位
    try {
        if (!untieFile.isDirectory()) { // 如果要保存分割文件地址不是路径
            untieFile.mkdirs(); // 创建该路径
        }
        size = size * filesize;
        long length = (int) commFile.length(); // 获取文件大小
        int num =(int)( length / size); // 获取文件大小除以MB的得数
        int yu = (int)(length % size); // 获取文件大小与MB相除的余数
